...
class AdminLogBehavior extends Behavior
{
    ...
    public function beforeAction($event)
    {
        if ($this->isActive($this->owner->action)) {
            ...
            Event::on(ActiveRecord::class, BaseActiveRecord::EVENT_AFTER_UPDATE, [$this, 'afterModelChanged']);
    ...
    public function afterModelChanged($event)
    {
        ...
        if ($event->name === ActiveRecord::EVENT_AFTER_INSERT) {
            ...
            $type = EnumLogType::TYPE_UPDATE;
        } elseif ($event->name === ActiveRecord::EVENT_AFTER_DELETE) {
            $type = EnumLogType::TYPE_DELETE;
        }
    ...
    protected function getDesc($log,$event)
    {
        if ($desc = $this->getSpecificDesc($this->owner->action)) {
            return "{$log->username}{$desc}";
        }
        $typeText = EnumLogType::getText($log->type);
        $changeDesc= $this->getChangedAttrDesc($event);
        $tablePk  = $event->sender->getPrimaryKey();
        return  "{$log->username}{$typeText}了表{$log->table_name}:[{$tablePk}]的 {$changeDesc}";
    ...
    protected function getSpecificDesc($action)
    {
        $id = $this->getActionId($action);
        foreach ($this->adminLogDesc as $pattern => $desc) {
            if (StringHelper::matchWildcard($pattern, $id)) {
                $descMatch = true;
            }
        }
